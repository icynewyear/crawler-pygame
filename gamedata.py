level_0 = {
    'terrain': 'data/level0/level0._terrain.csv',
    'player': 'data/level0/level0._player.csv',
    'ladders': 'data/level0/level0._ladders.csv',
    'bridges': 'data/level0/level0._bridges.csv',
    'waterfalls': 'data/level0/level0._waterfalls.csv',
    'enemies': 'data/level0/level0._enemies.csv',
    'doors': 'data/level0/level0._doors.csv',
    'coins': 'data/level0/level0._coins.csv',
    'chests': 'data/level0/level0._chests.csv',
    'constraints': 'data/level0/level0._constraints.csv',
    'items': 'data/level0/level0._items.csv',
    'spikes': 'data/level0/level0._spikes.csv',
    'traps': 'data/level0/level0._traps.csv',
    'pullchains': 'data/level0/level0._pullchains.csv',
}

levels = {
    0: level_0
}

